...
async def execute(ctx, params):
    ...
    if not params:
        ...
        if not ctx['admin'] and ctx['guild'].id != support_server_id:
            ...
            e.description = (
                "Je suis un bot qui vous permet de créer dynamiquement et à l'infini des canaux vocaux selon vos besoins, "
                "et les supprimer automatiquement dès qu'ils ne sont plus utilisés.\n\n"
                "Rejoignez simplement un de mes canaux vocaux et je vais en créer un nouveau pour vous et vous y déplacer."
            )
            text = (
                " ·  **<PREFIX>lock** - "
                "Verrouillez la limite d'utilisateurs de votre canal vocal pour que plus personne ne puisse se joindre. "
                "Utilisez **<PREFIX>unlock** pour supprimer la limite.\n\n"
                " ·  **<PREFIX>limit `N`** - "
                "Définissez la limite de votre chaîne."
                "Utilisez **<PREFIX>unlock** pour supprimer la limite.\n\n"
                " ·  **<PREFIX>private** - "
                "Rendez votre chaîne vocale privée, empêchant quiconque de vous rejoindre directement. "
                "Crée un canal \"⇩ Rejoignez {}\" au-dessus du vôtre CanalVocal.\n\n"
                " ·  **<PREFIX>kick `@USER`** - "
                "Lancez un vote pour supprimer quelqu'un de votre chaîne.\n\n"
                " ·  **<PREFIX>transfer `@USER`** - "
                "Transférez la propriété de votre chaîne à quelqu'un d'autre.\n\n".format(
                    esc_md(author.display_name)
                )
            )
            if ctx['gold']:
                text += (
                    " ·  **<PREFIX>name** - Modifier le nom de votre canal vocal.\n\n"
                    " ·  **<PREFIX>nick** - Changer le nom de la chaîne.\n\n"
                    " ·  **<PREFIX>bitrate** - Définissez un débit binaire personnalisé pour tout le serveur (en kbps) qui sera "
                    "utilisé pour toutes les chaînes que vous rejoignez.\n\n"
                )
            text += (
                " ·  **<PREFIX>invite** - Invitez-moi sur un autre serveur!\n\n"
                " ·  **<PREFIX>help `command`** - Obtenez plus d'informations sur une commande particulière."
            )
            text = text.replace('<PREFIX>', ctx['print_prefix'])
            e.add_field(name="Commandes:", value=text)
            try:
        ...
        for i, s in enumerate(sections):
            ...
            if s.startswith("\n**-- Commandes --**\n") and can_embed:
                ...
                for j, p in enumerate(parts):
                    ...
                    try:
                        await channel.send(content=p[0].replace("Commandes --**", "Commandes --**\n"), embed=embed)
                    except discord.errors.Forbidden:
                        log("Forbidden to echo", channel.guild)
                        await dm_user(
                            author,
